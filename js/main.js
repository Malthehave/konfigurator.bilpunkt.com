jQuery(document).ready(function($){
	...
	ProductBuilder.prototype.createSummary = function() {
		var bil = getQueryVariable("bil")
		// var bil = window.location.toString();
		// bil = bil.substring(bil.indexOf('bil'));
			var self = this;
	        self.queryParams = {
	            color : '',
	            options : [],
	        };
	        self.finalQuery = 'http://www.bilpunkt.com/email/tilbud/?bil=' + bil + '&motor=' + engine + '&';
			...
			this.steps.each(function(){
				...
				if( $(this).data('selection') == 'colors' ) {
					...
					if( selectedOptions.length == 0 ) {
						optionsContent = '<li><p>Ny bil</p></li><li><p>Brugt bil</p></li>';
					} else {
						selectedOptions.each(function(){
							optionsContent +='<li><p>'+$(this).find('p').text()+'</p></li>';
	                        self.queryParams.options.push($(this).find('p').text());
						});
					}
	        ...
	        Object.keys(self.queryParams).forEach(function (key) {
	           self.finalQuery += key + '=' + self.queryParams[key]+'&';
	        });
	        this.secondaryNavigation.find('.buy').find('a').attr('href', self.finalQuery);
		}
